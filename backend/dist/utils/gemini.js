...
function getClient() {
    return new generative_ai_1.GoogleGenerativeAI(keys[currentKeyIndex]);
}
function rotateKey() {
    ...
    for (const modelName of MODELS) {
        for (let attempt = 0; attempt < keys.length; attempt++) {
            try {
                ...
                logger_1.default.info(`Graded successfully — model: ${modelName}, key: ${currentKeyIndex}`);
                ...
                logger_1.default.warn(`Model ${modelName} key ${currentKeyIndex} failed (attempt ${attempt + 1}): ${err.message}`);
                rotateKey();
                if (attempt < keys.length - 1)
                    await sleep(1000);
            }
        }
        logger_1.default.warn(`All keys failed for model ${modelName} — trying next model...`);
        await sleep(3000);
    }
    // ═══ Fallback: Groq ═══
    logger_1.default.warn('All Gemini models failed — trying Groq...');
    try {
        const results = await gradeWithGroq(questions);
        logger_1.default.info('Graded successfully with Groq');
        return results;
    }
    catch (err) {
        logger_1.default.error(`Groq also failed: ${err.message}`);
    }
    // ═══ Fallback نهائي ═══
    logger_1.default.error('All models and keys failed — using fallback scores');
...
async function gradeWithGroq(questions) {
...
function buildGroqPrompt(questions) {
